...
const Sidebar = ({
  ...
  const handleNewChat = async () => {
    // only allow if current chat has messages (already saved)
    if (!hasMessages) return;

    // create placeholder (no DB save yet)
    const placeholder = { sessionId: null, title: "New Chat" };

    setSessions((prev) => [placeholder, ...prev]);
    onSelectSession(null); // sessionId = null until first message
  };
  ...
  return (
    ...
    >
      ...
      <div className="p-3 flex items-center justify-between bg-gray-800/50 border-b border-gray-700">
        {isOpen && (
          <h2 className="text-lg font-semibold truncate">Chat Sessions</h2>
        )}
        <button
